...
class ThinFilmLayer:
    ...
    def __init__(self, material, thickness, n_points, min_wavelength, max_wavelength):
        ...
        # Compute the path of the database_nk folder relative to this file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        database_dir = os.path.join(current_dir, "database_nk")
        
        filename = self.find_file_insensitive(database_dir, f"{material}.csv")
        if filename is not None:
            # read CSV file from the absolute path
